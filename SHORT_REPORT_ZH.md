...
模型结构包括 XLM-R encoder、第一个 token 的表示、dropout，以及一个线性的
multi-label DDC classification head。训练时使用 `BCEWithLogitsLoss`，然后把
sigmoid 概率通过和其他 DDC 模型相同的 threshold + Top-K 逻辑转换为最终 DDC
预测。

已完成的全量评估使用 1 个 epoch、`batch-size=16`、`max-length=256`，并使用
...
- XLM-R 评估设置：1 epoch，batch size 16，max length 256
- XLM-R 在 Apple Silicon MPS 上的训练耗时：39,541.8 秒，约 10.98 小时
- XLM-R 最终 training loss：0.0298
- XLM-R 校准阈值：0.325
...
XLM-RoBERTa 的结果接近 Logistic Regression，并且明显优于 MLP。但是在当前
设置下，它仍然没有超过 Logistic Regression。一个可能原因是 Transformer 只
读取前 256 个 subword tokens，而 TF-IDF 可以使用完整摘要中的词和 n-gram
信息。对于这种来自元数据的 DDC 分类任务，稀疏 TF-IDF 特征和线性分类器仍然
非常有效。

MLP 也明显优于两个简单 baseline，但低于 Logistic Regression 和 XLM-RoBERTa。
这说明仅仅增加模型复杂度并不一定带来提升，尤其是在输入仍然是稀疏 TF-IDF
表示的情况下。
...
对于 Transformer 模型，最大的限制是输入长度截断：`max-length=256` 意味着长
摘要后半部分的信息不会被模型看到。这可能是 XLM-RoBERTa 接近但没有超过
Logistic Regression 的原因之一。
...
- 继续改进 transformer-based 方法，尤其是长文本处理。
